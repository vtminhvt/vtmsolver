definition(
    name: "[Time]Thông báo hoạt động của công tắc/thiết bị ",
    namespace: "VTMS",
...
preferences {
    ...
    {
    	input("sw1","capability.switch",title:"Chọn công tắc điện bạn muốn điều khiển", multiple:true, required:true)     
    }
...
def timeC=	now()
    ...
    if (dk)
    {
        if(evt.value=="on") 
        {
            tb("${evt.displayName} đang mở/on: ${txt}")
        }
        else 	
        {
            tb("${evt.displayName} đã tắt/off: ${txt}")
       }
   }
}
